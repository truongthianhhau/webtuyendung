...
if(isset($_POST['post_job'])){
    ...
    // Get
    $filename = $filePath.$_FILES['post_company_file']['name'];
    ...
    // Get logo
    $imagename = $filePath.$_FILES['post_job_logo']['name'];
    ...
    $add_job = "INSERT INTO job (UploadFeaturedImage, JobTitle, JobRegion, Gender, JobType, education, other_benifits, publishedon, Responsibilities, experience, salary, application_deadline,
     email, location, job_description, CompanyName, UploadLogo, user_id)     VALUES('$imagename','$title', '$region', '$gender', '$type', '$education', '$benefits',
      '$publishedon','$responsibilities', '$experience', '$salary', '$application_deadline', '$email', '$location', '$job_desc','$post_company_name', '$filename', '$user_id' )";

    $conn->query($add_job);

    // if ($result === TRUE) {
    //     echo "Câu lệnh truy vấn đã được thực hiện thành công.";
    // } else {
